...
# Test Case 14: Trace patched function
# Trace a patched function and check that the live patch remains in
# effect.

set -e
. $(dirname $0)/klp_tc_functions.sh
klp_tc_init "Test Case 14: Trace patched function"

klp_tc_milestone "Compiling kernel live patch"
PATCH_KO="$(klp_create_patch_module tc_14 do_read_active_livepatch_id)"
PATCH_MOD_NAME="$(basename "$PATCH_KO" .ko)"

PATCH_DIR="/tmp/live-patch/tc_14"
...
if [ x"$(cat /sys/kernel/debug/klp_test_support/active_livepatch_id)" != xtc_14 ]; then
